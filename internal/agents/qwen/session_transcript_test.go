...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        send({"jsonrpc":"2.0","id":rid,"result":{
            "sessions":[{"sessionId":"ses_qwen_history","cwd":params.get("cwd",""),"title":"History"}]
        }})
    elif method == "session/load":
        sid = params.get("sessionId","")
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":sid,
            "update":{"sessionUpdate":"user_message_chunk","content":{"type":"text","text":"hello qwen"}}
        }})
        send({"jsonrpc":"2.0","method":"session/update","params":{
            "sessionId":sid,
...
`, python3)
	...
	client, err := qwen.New(qwen.Config{Dir: tmpDir})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	if got := result.Messages[0]; got.Role != "user" || got.Content != "hello qwen" {
		t.Fatalf("messages[0] = %+v, want user hello qwen", got)
	}
